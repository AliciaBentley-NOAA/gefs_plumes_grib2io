...
for i in range(len(members)):
  ...
  for j in range(len(fhours1)):
    if i==0:
      ...
      if j==0:
        #precip=grbs[42][0].data()-273.15
        precip=grbs.select(shortName='TMP',level='850 mb')[0].data-273.15
...
#write csv files
for k in range(len(slats)):
  f = open("GEFS"+slist[k]+ymdh+"850t.csv","wt")
  try:
    writer = csv.writer(f)
