...
for i in range(len(members)):
  ...
  for j in range(len(fhours1)):
    if i==0:
      ...
      grbs = grib2io.open('/lfs/h1/ops/prod/com/gefs/v12.3/gefs.'+str(ymd)+'/'+str(hour).zfill(2)+'/atmos/pgrb2bp5/ge'+members[i]+'.t'+str(hour).zfill(2)+'z.pgrb2b.0p50.f'+str(fhours1[j]).zfill(3), mode='r')
      #grib message order changes from f00 to f03 to f06
      if j==0:
        #precip=(grbs[261][0].data()-273.15)*1.8+32
        precip=(grbs.select(shortName='DPT',level='2 m above ground')[0].data()-273.15)*1.8 + 32
        precip=np.asarray(precip[::-1,:])
      elif j==1:
        #precip=(grbs[262][0].data()-273.15)*1.8+32
        precip=(grbs.select(shortName='DPT',level='2 m above ground')[0].data()-273.15)*1.8 + 32
        precip=np.asarray(precip[::-1,:])
      else:
        #precip=(grbs[262][0].data()-273.15)*1.8+32
    ...
    #get GFS data
      ...
      if j==0:
        #precip=(grbs[582][0].data()-273.15)*1.8+32
        precip=(grbs.select(shortName='DPT',level='2 m above ground')[0].data()-273.15)*1.8 + 32
        precip=np.asarray(precip[::-1,:])
      elif j==1:
        #precip=(grbs[583][0].data()-273.15)*1.8+32
        precip=(grbs.select(shortName='DPT',level='2 m above ground')[0].data()-273.15)*1.8 + 32
        precip=np.asarray(precip[::-1,:])
      else:
        #precip=(grbs[583][0].data()-273.15)*1.8+32
...
#write csv files
for k in range(len(slats)):
  f = open("GEFS"+slist[k]+ymdh+"2mdew.csv","wt")
  try:
    writer = csv.writer(f)
